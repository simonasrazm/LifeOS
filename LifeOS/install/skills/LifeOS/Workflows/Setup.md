...
7. **Enhancements menu** — present the seven optional components; let the user (or their AI, from context) pick any subset. Two deployers back this, both `isDevTree`-refusing, idempotent, backup-before-write:
   ...
   - **Codex → `bun Tools/InstallCodex.ts --apply`**: use the native adapter instead of `InstallHooks.ts`. It preserves custom `$CODEX_HOME/AGENTS.md` text and hook entries, links USER externally, maps supported lifecycle events, and reports unsupported events without creating inert registrations.
   - **Claude Code hooks → `bun Tools/InstallHooks.ts --apply`** (trust-gated; without `--apply` it only prints the plan): reads `install/hooks/hooks.json`, shows the EXACT change (file + settings-entry + event count), waits for explicit permission, backs up `settings.json`, merges additively per matcher bucket (idempotent via normalized-command dedup, preserves `type:"http"` verbatim).
...
8.5. **Wire the launch command (Core)** — the constitutional system prompt placed in step 4 (`LIFEOS_SYSTEM_PROMPT.md`) must reach the harness. **Codex:** `InstallCodex.ts` places a managed global `AGENTS.md` instruction that loads this file, so no shell alias is required. **Claude Code:** wire the permission-gated `lifeos` alias described in `INSTALL.md` step 7. **Other harnesses:** use their own system-prompt flag or context-file mechanism.
