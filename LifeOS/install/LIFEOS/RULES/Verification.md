...
> **This file is the ONE home of the seven incident-derived verification rules.** The system prompt keeps the constitutional core resident (self-check, the real-browser mandate, the "should work" ban, confidence-requires-source) and points here; the Algorithm's claims 8 and 16 bind these rules into runs; nothing else restates them. Load when verifying web/UI output, when a claim is about appearance, when deleting or replacing live infra, when verifying anything that must expire or propagate, or when the verifier is unavailable. Enforced by `hooks/VerificationGate.hook.ts` + `hooks/AlgorithmNudge.hook.ts`. Incident narratives live in `LIFEOS/MEMORY/LEARNING/INCIDENTS/` (private tree; the self-improvement loop's evidence base, beside REFLECTIONS and FAILURES).

Browser-verify all web output through the active harness's **native real-browser control** when available; otherwise use **Interceptor**. The evidence boundary is a real browser using the relevant session, not one product name: Codex desktop's native browser extension qualifies, as does Interceptor on a harness without an equivalent native surface. Preserve each carrier's isolation contract—agent-created isolated tabs by default, and explicit claiming before controlling an existing user tab. Standalone/headless Playwright is BANNED as substitute evidence; a harness-native API exposing Playwright-style locators over its real-browser connection is allowed. "curl returns 200" is not browser verification. Every time you create, fix, deploy, or claim anything works on the web, use the active real-browser verifier. No exceptions.

## The seven rules

1. **Modality fidelity** — the probe must exercise the SAME path the user does: a Web/UI claim closes only on a real browser navigation to the actual URL through the active real-browser verifier. A `curl`, a DOM read of a *different* page, or a check of a sibling path is a different request and proves nothing about what the browser renders at the path in question — curl and a real browser can literally receive different pages. "I curled it and got the right thing" is not "the user's browser gets the right thing."

2. **Unavailable verifier ⇒ DEFER, never substitute** — when the active harness's native browser and Interceptor fallback are both unavailable, a web claim is NOT verified. Say "deployed, not browser-verified", mark the ISC `[DEFERRED-VERIFY]`, and do NOT claim live / works / shipped / locked-down. Restore a real-browser carrier, wait, or hand the human-only step over — never relabel weaker evidence as verification.

3. **Appearance ≠ existence — look at the pixels, from a faithful renderer** — a DOM read proves an element EXISTS at coordinates; it proves NOTHING about how it looks. The pixels only count when the render path has fidelity to what the user's device paints. Any claim about *appearance* — a logo/image renders, is centered, transparent, the right color, "looks right" — closes ONLY on a **non-degenerate pixel image you actually looked at** (Read/view the image; a black frame is not a look). **View every asset before wiring it in.** When Interceptor is the carrier, its DOM-render capture is a reconstruction that does not paint UA default widget chrome, CSS pseudo-element content, or property-set input values; contrast and native-control claims therefore need its `--pixel` capture or a `getComputedStyle` probe, and `Capture.sh` retains its blank-frame guard. A native harness screenshot captured from the real tab is the pixel surface for that carrier. `VerificationGate` (T3) blocks an appearance-success claim unless a pixel image was captured and viewed after the last frontend edit.

4. **Reproduce before fixing** — for ANY reported UI or page bug, OPEN THE PAGE WITH THE ACTIVE REAL-BROWSER VERIFIER FIRST — before reading code, before theorizing, before writing fixes. Use the harness-native carrier when available and Interceptor otherwise. Check console errors. Check network 404s. See the failure with your own eyes. Code analysis without reproduction is speculation, not debugging.
