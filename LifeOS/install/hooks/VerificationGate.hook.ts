...
const BLOCK_MSGS: Record<string, (unit: string, ev: string) => string> = {
  T1: (u, ev) => `WEB-DEPLOY VERIFICATION GAP [VerificationGate/T1]. You claimed: "${u}". The transcript shows the deployed thing was never probed after the deploy — ${ev}. Deployed ≠ live. Do ONE, then restate: (a) probe the deployed origin through the active real-browser verifier, or use curl for a non-visual HTTP claim, after the deploy; or (b) downgrade honestly ("deployed, not verified live"). This gate reads the transcript's real tool calls, not your wording — rewording won't pass it; verifying or downgrading will.`,
